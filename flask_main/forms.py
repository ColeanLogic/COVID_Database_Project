...
from wtforms import StringField, SubmitField, TextAreaField, RadioField, DecimalField, SelectField, IntegerField, PasswordField
...
class PatientForm(FlaskForm):
    ...
    admitted = DateField(
        'Date Admitted ', format='%Y-%m-%d', validators=[Optional()])
    discharged = DateField(
        'Date Discharged ', format='%Y-%m-%d', validators=[Optional()])
...
class CaseForm(FlaskForm):
    ...
    patient_id = DecimalField('Patient ID* ', places=0,
                              validators=[DataRequired()])
    county_id = DecimalField('County ID* ', places=0,
                             validators=[DataRequired()])
    hospital_id = DecimalField(
        'Hospital ID* ', places=0, validators=[DataRequired()])
...
class CaseEditForm(FlaskForm):
    ...
    patient_id = DecimalField(
        'Patient ID* ', places=0, validators=[Optional()])
    county_id = DecimalField('County ID* ', places=0, validators=[Optional()])
    hospital_id = DecimalField(
        'Hospital ID* ', places=0, validators=[Optional()])
    status = RadioField("Status ", choices=[
        ("in hospital ", "In Hospital"),
        ("at home", "At Home"),
        ("recovered", "Recovered"),
...
class registrationForm(FlaskForm):
    ...
    role = SelectField('Role', choices=[
        ...
        ("media", "Media")], validators=[DataRequired()])
    submit = SubmitField('Submit')
